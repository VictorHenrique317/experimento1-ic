...
class FuzzyTuple
{
 public:
  ...
  FuzzyTuple(vector<unsigned int>& tuple, const float membership);
  FuzzyTuple(const vector<vector<unsigned int>::const_iterator>& tupleIts, const float membership);
  ...
  float getMembershipSquared() const;
  ...
  void shiftMembership(const float shift);
