...
AbstractRoughTensor* AbstractRoughTensor::makeRoughTensor(vector<FuzzyTuple>& fuzzyTuples, const float densityThreshold, const float shift)
{
  if (Trie::is01)
    {
      if ((8 * sizeof(float) + 1) * getTensorArea() < 8 * (3 * sizeof(unsigned int*) + sizeof(unsigned int) * (ids2Labels.size() + 1) + sizeof(float)) * fuzzyTuples.size())
      	{
  ...
  if ((sizeof(unsigned int) + sizeof(float)) * getTensorArea() < (3 * sizeof(unsigned int*) + sizeof(unsigned int) * (ids2Labels.size() + 2) + sizeof(float)) * fuzzyTuples.size())
    {
      // Dense storage (including the rough tensor) takes less space, assuming the sparse storage would only use sparse tubes
      return new DenseRoughTensor(fuzzyTuples, shift);
    }
...
AbstractRoughTensor* AbstractRoughTensor::makeRoughTensor(const char* tensorFileName, const char* inputDimensionSeparator, const char* inputElementSeparator, const float densityThreshold, const bool isVerbose)
{
...
AbstractRoughTensor* AbstractRoughTensor::makeRoughTensor(const char* tensorFileName, const char* inputDimensionSeparator, const char* inputElementSeparator, const float densityThreshold, const float shift, const bool isVerbose)
{
  vector<FuzzyTuple> fuzzyTuples = getFuzzyTuples(tensorFileName, inputDimensionSeparator, inputElementSeparator, isVerbose);
  return makeRoughTensor(fuzzyTuples, densityThreshold, shift);
}

void AbstractRoughTensor::setOutput(const char* outputFileName, const char* outputDimensionSeparatorParam, const char* outputElementSeparatorParam, const char* hierarchyPrefixParam, const char* hierarchySeparatorParam, const char* sizePrefixParam, const char* sizeSeparatorParam, const char* areaPrefixParam, const bool isPrintLambdaParam, const bool isSizePrintedParam, const bool isAreaPrintedParam)
{
#ifdef DETAILED_TIME
#ifdef GNUPLOT
...
vector<FuzzyTuple> AbstractRoughTensor::getFuzzyTuples(const char* tensorFileName, const char* inputDimensionSeparator, const char* inputElementSeparator, const bool isVerbose)
{
  ...
  if (fuzzyTuplesAndIs01.first.empty())
    {
      throw UsageException(("No fuzzy tuple in " + string(tensorFileName) + '!').c_str());
    }
...
void AbstractRoughTensor::setMetadata(vector<FuzzyTuple>& fuzzyTuples, const float shift)
{
  // Sparse tensor
  const unsigned long long area = getTensorArea();
  // Shift the memberships and compute and compute the quadratic error of the null model
  if (Trie::is01)
    {
  ...
  float unitDenominator = sqrt(nullModelQuadraticError);
  ...
  for (vector<string>& ids2LabelsInDimension : ids2Labels)
    {
      ...
      dimensions.emplace_back(nbOfElements, dimensionId);
      // Computing positive and negative memberships of the elements
      vector<pair<float, unsigned int>> elementPositiveMemberships;
      ...
      for (unsigned int id = 0; id != nbOfElements; ++id)
	{
      ...
      vector<float> elementNegativeMemberships(nbOfElements, shift * (area / nbOfElements)); // assumes every membership null and correct that in the loop below
      if (Trie::is01)
	{
	  const float oneMinusShift = 1. - shift;
	  for (const FuzzyTuple& fuzzyTuple : fuzzyTuples)
	    {
      ...
      else
	{
      ...
      sort(elementPositiveMemberships.begin(), elementPositiveMemberships.end(), [](const pair<float, unsigned int>& elementPositiveMembership1, const pair<float, unsigned int>& elementPositiveMembership2) {return elementPositiveMembership1.first < elementPositiveMembership2.first;});
      ...
      for (const pair<float, unsigned int>& elementPositiveMembership : elementPositiveMemberships)
	{
      ...
      for (FuzzyTuple& fuzzyTuple : fuzzyTuples)
	{
      ...
      const float maxNegativeMembership = *max_element(elementNegativeMemberships.begin(), elementNegativeMemberships.end());
      if (maxNegativeMembership > elementPositiveMemberships.back().first)
	{
...
#endif
  setUnit(static_cast<float>(numeric_limits<int>::max()) / unitDenominator);
}

void AbstractRoughTensor::setMetadata(vector<vector<pair<float, unsigned int>>>& elementPositiveMemberships, const vector<vector<float>>& elementNegativeMemberships)
{
  // Dense tensor
  float unitDenominator = sqrt(nullModelQuadraticError);
  vector<pair<unsigned int, unsigned int>> dimensions;
  ...
  vector<vector<float>>::const_iterator elementNegativeMembershipsInDimensionIt = elementNegativeMemberships.begin();
  for (vector<string>& ids2LabelsInDimension : ids2Labels)
    {
      sort(elementPositiveMembershipsInDimensionIt->begin(), elementPositiveMembershipsInDimensionIt->end(), [](const pair<float, unsigned int>& elementPositiveMembership1, const pair<float, unsigned int>& elementPositiveMembership2) {return elementPositiveMembership1.first < elementPositiveMembership2.first;});
      const unsigned int nbOfElements = ids2LabelsInDimension.size();
      dimensions.emplace_back(nbOfElements, dimensionId++);
      // Computing the new ids, in increasing order of the positive membership (for faster lower_bound in SparseFuzzyTube::sumOnSlice and for Trie::maxExtension to choose the element with the greatest membership in case of equality) and reorder ids2LabelsInDimension accordingly
      vector<string> newIds2LabelsInDimension;
      newIds2LabelsInDimension.reserve(nbOfElements);
      for (const pair<float, unsigned int>& elementPositiveMembership : *elementPositiveMembershipsInDimensionIt)
	{
	  newIds2LabelsInDimension.emplace_back(std::move(ids2LabelsInDimension[elementPositiveMembership.second]));
	}
      ids2LabelsInDimension = std::move(newIds2LabelsInDimension);
      const float maxNegativeMembership = -*min_element(elementNegativeMembershipsInDimensionIt->begin(), elementNegativeMembershipsInDimensionIt->end());
      if (maxNegativeMembership > elementPositiveMembershipsInDimensionIt->back().first)
	{
	  if (maxNegativeMembership > unitDenominator)
	    {
	      unitDenominator = maxNegativeMembership;
