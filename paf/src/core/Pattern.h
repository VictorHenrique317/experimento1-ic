...
class Pattern
{
 public:
  ...
  Pattern(const vector<vector<unsigned int>>& nSet);

  Pattern& operator=(Pattern&& otherPattern);

  static void grow(const char* patternFileName, const char* patternDimensionSeparator, const char* patternElementSeparator, const AbstractRoughTensor* roughTensor, Trie& tensor, const unsigned int nbOfGrownPatterns, const bool isInputMaximal, const bool isIntermediaryPatternsOutput, const bool isOnlyGrow, const bool isVerbose);
