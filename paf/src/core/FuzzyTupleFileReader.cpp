// Copyright (C) 2018,2019,2021 Loïc Cerf (lcerf@dcc.ufmg.br)
...
bool FuzzyTupleFileReader::parseLine(const tokenizer<char_separator<char>>::const_iterator dimensionBegin, const tokenizer<char_separator<char>>::const_iterator dimensionEnd)
{
  ...
  try
    {
      membership = lexical_cast<float>(*membershipIt);
      if (membership == 0)
	{
	  return false;
	}
    ...
    {
      throw DataFormatException(tensorFileName.c_str(), lineNb, ("the membership, " + *membershipIt + ", should be a float in [0, 1]!").c_str());
    }
