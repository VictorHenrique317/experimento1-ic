...
class AbstractShift
{
 public:
  ...
  virtual float getShift(const vector<unsigned int>& tuple) const = 0;
  virtual float getAverageShift(const vector<vector<unsigned int>>& nSet) const = 0;

  virtual void setNewDimensionOrderAndNewIds(const vector<unsigned int>& old2NewDimensionOrder, const vector<vector<pair<float, unsigned int>>>& elementPositiveMemberships) = 0;
