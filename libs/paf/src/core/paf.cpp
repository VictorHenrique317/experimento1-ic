...
int main(int argc, char* argv[])
{
  ...
  try
      ...
      if (vm.count("help"))
	{
      ...
      if (vm.count("version"))
	{
	  cout << "paf version 0.17.6\nCopyright (C) 2021 Loïc Cerf.\nLicense GPLv3+: GNU GPL version 3 or later <https://gnu.org/licenses/gpl.html>\nThis is free software: you are free to change and redistribute it.\nThere is NO WARRANTY, to the extent permitted by law.\n";
	  return EX_OK;
	}
      ifstream optionFile;
      if (vm.count("opt"))
      ...
      if (isVerbose)
	{
      ...
      AbstractRoughTensor::setOutput(outputFileName.c_str(), vm["ods"].as<string>().c_str(), vm["oes"].as<string>().c_str(), vm["hp"].as<string>().c_str(), vm["hs"].as<string>().c_str(), vm["sp"].as<string>().c_str(), vm["ss"].as<string>().c_str(), vm["ap"].as<string>().c_str(), vm.count("pl"), vm.count("ps"), vm.count("pa"));
      isOnlyGrow = !vm.count("amalgamate");
      Pattern::grow(patternFileName.c_str(), vm["pds"].as<string>().c_str(), vm["pes"].as<string>().c_str(), roughTensor, tensor, nbOfGrownPatterns, vm.count("max"), !vm.count("ni"), isOnlyGrow, isVerbose);
    }
