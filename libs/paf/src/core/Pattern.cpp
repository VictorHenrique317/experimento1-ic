...
Pattern::Pattern(const vector<vector<unsigned int>>& nSetParam): nSet(nSetParam.size()), membershipSum(), dimensionIdOfNextElement(), nextElement(), membershipSumOnNextElement(), rssDiff()
{
  vector<vector<unsigned int>>::const_iterator externalNSetDimensionIt = nSetParam.begin();
  if (Trie::is01)
    {
	...
	{
	  membershipSum *= externalNSetDimensionIt->size();
	  nSet[internalDimensionId] = *externalNSetDimensionIt++;
	}
  ...
  if (bestAdjustedNextMembershipSum <= membershipSum)
    {
      ...
      const unsigned long long patternArea = area();
