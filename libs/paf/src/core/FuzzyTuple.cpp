...
FuzzyTuple::FuzzyTuple(vector<unsigned int>& tupleParam, const float membershipParam): tuple(std::move(tupleParam)), membership(membershipParam)
{
}

FuzzyTuple::FuzzyTuple(const vector<vector<unsigned int>::const_iterator>& tupleIts, const float membershipParam): tuple(), membership(membershipParam)
{
...
float FuzzyTuple::getMembership() const
{
  return membership;
}

float FuzzyTuple::getMembershipSquared() const
{
...
void FuzzyTuple::shiftMembership(const float shift)
{
  membership -= shift;
}
