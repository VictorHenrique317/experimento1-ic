...
{
 public:
  ConstantShift(const float constant);

  float getShift(const vector<unsigned int>& tuple) const;
  float getAverageShift(const vector<vector<unsigned int>>& nSet) const;

  void setNewDimensionOrderAndNewIds(const vector<unsigned int>& old2NewDimensionOrder, const vector<vector<pair<float, unsigned int>>>& elementPositiveMemberships);
