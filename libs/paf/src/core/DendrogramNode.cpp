	  ...
	  while (dendrogramFrontier.size() != 1)
	    {
	      ...
	      CompletionMessage<unsigned long long>::print(static_cast<long long>(candidates.size()) * (dendrogramFrontier.size() + 1));
	    }
	  CompletionMessage<unsigned long long>::done();
	}
      else
